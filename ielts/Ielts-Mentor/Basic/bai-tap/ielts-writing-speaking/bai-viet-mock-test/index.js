...
app.get("/api/health", (req, res) => {
    return res.status(200).json({
        success: true,
        server: "IELTS Writing AI",
        environment: IS_VERCEL ? "Vercel" : IS_RENDER ? "Render" : "Local",
...
// ============================================================
// STATIC (cho cả Vercel, Render, Local)
// ============================================================

// Phục vụ file tĩnh ở BASE_PATH (trên Vercel BASE_PATH = "")
app.use(BASE_PATH, express.static(ROOT_DIR));
...
app.get(`${BASE_PATH}/api/ai-status`, async (req, res) => {
    try {
        ...
        console.log("🤖 Model:", requestedModel || DEFAULT_MODEL);

        const result = await testGeminiConnection(requestedModel);
        return res.status(200).json({
            success: true,
            connected: true,
            model: result.model,
            message: "Gemini AI đã kết nối thành công.",
...
app.post(`${BASE_PATH}/api/check-writing`, async (req, res) => {
    ...
    try {
        ...
        console.log("📌 Topic:", topic || "(không có)");
        console.log("📌 Title:", title || "(không có)");
        console.log("📌 Outline:", outline ? "Có" : "Không");
        console.log("🤖 Model:", model || DEFAULT_MODEL);

        // VALIDATE
        if (typeof writing !== "string" || !writing.trim()) {
        ...
        const cleanWriting = writing.trim();

        // WORD COUNT
        const wordCount = cleanWriting.split(/\s+/).filter(Boolean).length;
        console.log("📊 WORD COUNT:", wordCount);

        // CALL GEMINI
        const result = await checkWritingByGemini(
            topic,
            cleanWriting,
            45000,
            model
        ...
        return res.status(500).json({
            success: false,
            message: error.message || "Không thể chấm bài bằng Gemini AI.",
...
// Trên Vercel, route gốc "/" trả về index.html
if (IS_VERCEL) {
    app.get("/", (req, res) => {
        return res.sendFile(INDEX_PATH);
    });
} else {
    // Local / Render: trả về index.html tại BASE_PATH
...
app.use("/api", (req, res) => {
    console.error("❌ API NOT FOUND:", req.method, req.originalUrl);
...
if (!IS_VERCEL) {
    app.listen(PORT, "0.0.0.0", () => {
        ...
        console.log("📋 GEMINI MODELS:", SUPPORTED_MODELS.join(", "));
        console.log("🖥️ ENVIRONMENT:", IS_RENDER ? "Render" : "Local");
        console.log("========================================");
        console.log("");
    });
}
